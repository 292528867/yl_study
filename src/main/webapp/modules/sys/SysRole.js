...
//	                destroy: 'POST'
		...
		me.rolepermissionstore = Ext.create('Ext.data.Store', {
			...
			listeners : {
				beforeload : function(store, operation, eOpts) {
					var roleId = me.storeparams["treeroleId"];
	        		...
	        		Ext.Msg.confirm('确认', '请问您确认删除该角色吗？', function(bt) {
	        			if (bt == 'yes') {
	        				...
	        				for (var i = 0; i < records.length; i++) 
	        					roleIds.push(records[i].data['roleId']);
	        				Ext.Ajax.request({
	        					url : '/busbatterysystem/web/security/sysRoleController/invalidroles',
	        					methods : 'POST',
	...
	createTree_1 : function(roleId) {
		var me = this;
		
		me.departmentusertreestore.getRootNode().removeAll();
		me.departmentusertreestore.load();
		
		var tree = Ext.create("Ext.tree.Panel", {
			store : me.departmentusertreestore,
			rootVisible : false,
			useArrows : true,
			frame : true, 
			dockedItems: [{
